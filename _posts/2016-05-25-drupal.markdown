...
Ctools Plugins
==================
Ctools module is a contributed module to provide a set of API and function, like Page Manager, Context tools, Access tools (visibility rules), configuration exportability, form wizards.

And the `Plugins` is one function this module provide, With this tool we could extend the modules or themes with `.inc' file. There're 3 kinds of include file: layouts (in theme, feature), content type and styles (in modules).

`content_types` ctools plugin exits in module, it allow us to create our own panel pane that display dynamic content. In the pane, we could even setup a form to do the configuration.

In `.module` file, we could use `HOOK_ctools_plugin_directory` to register the plugin directory:
{% highlight php%}
/**
...
In the `.inc` file itself, we will do 2 things: how to render, how to configuration (optional).
{% highlight php%}
$plugin = array(
  'title' => t('TESTING'),
...
`content type` plugins could also be living in features, the create action will still keep the registration and `.inc` file. And we avoid putting in the theme level, as the pane could be used across themes.
